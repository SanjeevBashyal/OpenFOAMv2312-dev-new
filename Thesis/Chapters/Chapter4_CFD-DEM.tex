...
\end{equation}
\equations{Reynolds Decomposition of Velocity}
...
\end{equation}
\equations{Boussinesq Hypothesis for Reynolds Stress}
...
\end{equation}
\equations{VOF Mixture Density}
\begin{equation}
	\mu = \alpha \mu_1 + (1-\alpha) \mu_2
\end{equation}
\equations{VOF Mixture Viscosity}
...
\end{equation}
\equations{Newton's Second Law for Particle Translation}
\begin{equation}
...
\end{equation}
\equations{Newton's Second Law for Particle Rotation}
...
\end{equation}
\equations{Euler Integration for Velocity}
\begin{equation}
	\mathbf{x}_{t+\Delta t} = \mathbf{x}_t + \mathbf{v}_{t+\Delta t} \Delta t
\end{equation}
\equations{Euler Integration for Position}
...
\begin{equation}
	F_n = \begin{cases} 
		k_1 \delta & \text{Loading } (\delta \ge 0) \\
		k_2 (\delta - \delta_p) & \text{Unloading } (\dot{\delta} < \delta_{max})
	\end{cases}
\end{equation}
\equations{Walton-Braun Hysteretic Normal Force}
where $k_1$ and $k_2$ are loading and unloading stiffnesses, and $\delta_p$ is the residual overlap. This hysteresis accounts for energy dissipation (plasticity).
...
\end{equation}
\equations{Coulomb Friction Model}
...
\end{equation}
\equations{IBM Solid Volume Fraction}
...
\end{equation}
\equations{IBM Solid Velocity Construction}
...
\begin{equation}
	\mathbf{F}_{fluid} = \sum_{cells} \left(-\nabla p V_{cell} \epsilon_{cell}\right) + \sum_{cells}\left(\mu \nabla^2 \mathbf{U} \cdot \epsilon_{\text {cell }} \cdot V_{\text {cell }}\right)
\end{equation}
\equations{IBM Fluid-Particle Force Integration}
