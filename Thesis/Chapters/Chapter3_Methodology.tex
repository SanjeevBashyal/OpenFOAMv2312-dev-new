...
\section{Fundamental Validation Test Cases}

To validate the developed solver, four fundamental test cases are established. These cases isolate specific physics: mechanical contact (DEM), fluid-structure interaction (CFD-DEM), tangential forces, and free-surface intersection.

\subsection{Test Case 1: DEM Validation - The Hysteretic Bounce}
...
\textbf{Analytical Target:}
\begin{enumerate}
    \item \textbf{Expected Coefficient of Restitution:} $e = \sqrt{10^5 / (4 \times 10^5)} = \sqrt{0.25} = 0.5$.
    \item \textbf{Expected Rebound Height:} $H_1 = H_0 \times e^2 = 1.0 \times (0.5)^2 = 0.25\,m$.
\end{enumerate}

\textbf{Validation Metrics:}
\begin{itemize}
    \item \textbf{Vertical Position ($z$) vs. Time:} The peak of the second bounce should be exactly $0.25m$.
    \item \textbf{Force vs. Overlap (Hysteresis Loop):} The normal force and penetration depth during impact should form a triangle-like shape, with slope $k_1$ during loading and $k_2$ during unloading.
\end{itemize}

\subsection{Test Case 2: CFD-DEM Validation - Terminal Velocity of a Settling Cube}
...
\textbf{Analytical Target:}
\begin{enumerate}
    \item \textbf{Net Driving Force:} $F_{net} = F_g - F_b = 0.15696 - 0.07848 = 0.07848 \, N$.
    \item \textbf{Expected Terminal Velocity ($v_t$):}
    \begin{equation}
    v_t = \sqrt{\frac{2(F_g - F_b)}{\rho_f A C_d}} \approx 0.611 \, m/s
    \end{equation}
    (Assuming $C_d \approx 1.05$ for a cube).
\end{enumerate}

\textbf{Validation Metrics:}
\begin{itemize}
    \item \textbf{Particle Velocity ($v_z$) vs. Time:} Should asymptotically approach $\approx 0.61 \, m/s$.
    \item \textbf{Fluid Velocity Field:} A recirculation zone (wake) should form behind the cube.
\end{itemize}

\subsection{Test Case 3: DEM Validation - Sliding Block on an Inclined Plane}
...
\textbf{Analytical Target:}
\begin{enumerate}
    \item \textbf{Expected Acceleration:}
    \begin{equation}
    a = 9.81 \times (\sin(30^\circ) - 0.3 \cos(30^\circ)) \approx 2.356 \, m/s^2
    \end{equation}
\end{enumerate}

\textbf{Validation Metrics:}
\begin{itemize}
    \item \textbf{Velocity ($v$) vs. Time:} The slope should be exactly $2.356$.
    \item \textbf{Tangential Force Magnitude:} Should equal $\mu m g \cos(\theta) \approx 2.55 \, N$.
\end{itemize}

\subsection{Test Case 4: CFD-DEM Validation - The Floating Cube Equilibrium}
...
\textbf{Analytical Target:}
\begin{enumerate}
    \item \textbf{Equilibrium Depth:} $h_{sub} = L (\rho_s/\rho_f) = 0.05\,m$ (Half-submerged).
    \item \textbf{Oscillation Period:} $T = 2\pi \sqrt{h_{sub} / g} \approx 0.448 \, s$.
\end{enumerate}

\textbf{Validation Metrics:}
\begin{itemize}
    \item \textbf{Vertical Position:} Should settle at $z = Y_{water} + (L/2) - h_{sub}$.
    \item \textbf{Volume Fraction Field:} Inside block $\epsilon=1$. At water line, cells should be partially solid and partially water.
\end{itemize}
