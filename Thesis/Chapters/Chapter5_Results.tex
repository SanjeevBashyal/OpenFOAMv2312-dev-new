...
\chapter{ RESULTS }
\label{chapter6}
\section{Comparative Analysis of RANS Turbulence Closure Schemes in SedFoam}

\subsection{Single Fluid Phase Flow near Solid Boundary}

The velocity distribution at the end of 1000s obtained from the simulation with turbulence model $k-\omega $ shows higher velocity gradient near the boundary than that of $k-\epsilon $ turbulence model.\par

\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/Chapters/C06/velocity_distribution_case_boundary.png}
	\caption{Velocity Distribution along vertical direction from solid boundary at t=1000s; $k-\omega $ turbulence model on the left and $k-\epsilon $ model on the right}
	\label{fig:Velocity Distribution along vertical direction from solid boundary at t=1000s}
\end{figure}

The turbulent kinetic energy is found to be higher for same average velocities for $k-\epsilon $ simulation than $k-\omega $ simulation which is observed from the higher values of $k$ in $k-\epsilon $ simulation shown in the figure below:\par

\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/Chapters/C06/k-distribution_case_boundary.png}
	\caption{Turbulent kinetic energy distribution along vertical direction from solid boundary at t=1000s; $k-\omega $ turbulence model on the left and $k-\epsilon $ model on the right}
	\label{fig:Turbulent kinetic energy distribution along vertical direction from solid boundary at t=1000s}
\end{figure}

Compared with DNS simulations performed by \parencite{mansour_direct_1999}, the velocity distribution $\left(\frac{u}{u_{max}}\right)$ resulted by $k-\omega $ model closely matches with DNS results with a NSE value of 0.97 whereas $k-\epsilon $ model resulted a NSE 0.93.\par

\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/Chapters/C06/velocity_graph_case_boundary.png}
	\caption{Comparison of velocity distribution obtained from simulation with DNS performed by \parencite{mansour_direct_1999}}
	\label{fig:Comparison of velocity distribution}
\end{figure}

\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/Chapters/C06/k-graph_case_boundary.png}
	\caption{Comparison of turbulent kinetic energy distribution normalized with wall shear stress, with DNS results}
	\label{fig:Comparison of turbulent kinetic energy distribution normalized with wall shear stress, with DNS results}
\end{figure}

The result is consistent with the literature which suggests that the $k-\epsilon $ model better simulates for flows with high Reynold's number whereas $k-\omega $ model better simulates the flow regime with high shear stresses \parencite{fylladitakis_kolmogorov_2018}.\par

\subsection{Scour Study under rigid wall pipe}

The scouring initiation in the bed just below the pipe happens with the fluid phase developing a streamline just beneath the pipe.\par

\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/Chapters/C06/phase_water_case_bed.png}
	\caption{Concentration plot of phase a (sediment) showing scouring initiation at t=0.5s}
	\label{fig:Concentration plot of phase a (sediment) showing scouring initiation at t=0.5s}
\end{figure}

The simulation with turbulence model $k-\omega $ well resolves the first vortex initiated from the top of the pipe whereas the vortex is poorly developed in $k-\epsilon $ model.\par

\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/Chapters/C06/velocity_case_bed.png}
	\caption{Velocity plot of Fluid phase b (water) at time t=1s}
	\label{fig:Velocity plot of Fluid phase b (water) at time t=1s}
\end{figure}

\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/Chapters/C06/velocity_case2_bed.png}
	\caption{Velocity plot of Fluid phase b (water) at time t=3s}
	\label{fig:Velocity plot of Fluid phase b (water) at time t=3s}
\end{figure}

The rate of sediment getting swept by the flow gradually increases for both simulations whereas the rate is higher for the case of $k-\epsilon $ turbulence model which is due to higher production and dissipation rate of turbulent kinetic energy for $k-\epsilon $ model.\par

\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/Chapters/C06/k_case_bed.png}
	\caption{Turbulent Kinetic Energy Plot at time t=3s}
	\label{fig:Turbulent Kinetic Energy Plot at time t=3s}
\end{figure}

\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/Chapters/C06/phase_water2_case_bed.png}
	\caption{Concentration plot of Fluid phase a (sediment) at time t=3s}
	\label{fig:Concentration plot of Fluid phase a (sediment) at time t=3s}
\end{figure}

\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/Chapters/C06/phase_water3_case_bed.png}
	\caption{Concentration plot of Fluid phase a (sediment) at time t=5s}
	\label{fig:Concentration plot of Fluid phase a (sediment) at time t=5s}
\end{figure}

Comparing with the bed scour obtained by the physical experiment performed by \parencite{mao_interaction_1986}, the bed simulated by SedFoam solver with fluid phase turbulence model $k-\omega $ has a better NSE value 0.72 than that of 0.27 produced by bed profile with fluid phase turbulence model $k-\epsilon $.\par

\begin{figure}[ht]
	\centering
	\includegraphics[width=1.0\textwidth]{Figures/Chapters/C06/bed_profile_case_bed.png}
	\caption{Comparison plot of Bed Profile obtained at time t=5s, between Sedfoam solver with fluid phase turbulence models $(k-\omega \& k-\varepsilon)$ and physical experiment done by \parencite{mao_interaction_1986}}
	\label{fig:Comparison plot of Bed Profile}
\end{figure}

Two phase flow near bed region is characterized large sediment concentration and high shear stresses due to which $k-\omega $ model is better turbulence model for fluid phase.\par

\section{OpenFOAM based CFD-DEM Solver: Free sedimentation of a particle in a static water column }
