...
The accurate modeling of sediment transport in rivers is a critical challenge due to the complex interplay between turbulent fluid flow and individual sediment particles, particularly in near-bed regions. Existing numerical models often struggle to capture the full spectrum of these interactions, leading to significant uncertainties in predicting sediment movement, erosion, and deposition. \par

Continuum models, such as those implemented in SedFoam \parencite{chauchat_sedfoam-20_2017}, treat sediment as a continuous phase. While effective for high sediment concentrations and large-scale morphological changes, this approach inherently limits their ability to resolve the kinematics of individual, non-spherical particles, their collisions, or interlocking effects. These granular-scale phenomena are vital for accurately representing bedload transport and the physics of the sediment bed. \par

Standard CFD-DEM solvers, like CFDEM and SediFoam \parencite{hager_parallel_2014,sun_sedifoam_2016}, offer a step towards resolving particle dynamics but frequently rely on simplified spherical particle representations or basic contact models. This simplification fails to capture the hydrodynamics of arbitrary-shaped particles, which are ubiquitous in natural river systems, thus limiting the fidelity of simulations involving realistic sediment grains.

More recently, arbitrary-shape solvers, such as OpenHFDIB-DEM \parencite{studenik_openhfdib-dem_2024}, have made significant strides by employing Immersed Boundary Methods (IBM) to represent complex particle geometries. However, a key limitation of these advanced solvers is their lack of integrated free-surface modeling capabilities. This omission renders them unsuitable for simulating open-channel flows, where the air-water interface plays a crucial role in flow dynamics and particle entrainment.

Addressing these multifaceted limitations requires the development of a coupled turbulence closure and sediment transport model that integrates advanced CFD-DEM techniques with adaptable computational frameworks, including robust free-surface handling. By leveraging OpenFOAM for its flexibility and incorporating advanced turbulence models and IBM for complex shapes, such a framework can provide improved predictive accuracy and computational efficiency for simulating sediment transport in river systems.

\section{Research Questions}

The research questions of the study are as follows:
\begin{itemize}
	\item How can the interactions between turbulent flow structures and particle transport processes be mathematically modeled in free surface flows?
	\item How well does the OpenFOAM based CFD-DEM solver that simulates free surface flows perform in predicting particle mixed flow under various conditions?
	\item What is the influence of the turbulence closure scheme on the simulations performed by the solver involving sediment transport dynamics?
...
\begin{itemize}
	\item To develop a mathematical model that combines RANS turbulence closure models with discrete element method to simulate particle transport processes during free surface water flows.
	\item To implement the model in OpenFOAM framework and to assess the predictive capabilities of the model.
	\item To identify the most suitable two equation standard RANS turbulence closure scheme for the solver for better prediction of sediment transport phenomenon near bed region in open channel flow.

\end{itemize}

\section{Significance of the study}
Accurate predictions of sediment-laden flows are essential for managing dynamic river systems, especially in regions with high sediment loads and turbulent conditions, such as Himalayan rivers \parencite{andermann_sediment_2012}. However, existing numerical models present a critical trade-off: continuum-based approaches and standard CFD-DEM solvers using spherical particles fail to resolve the granular-scale physics of arbitrarily shaped grains, while advanced shape-resolved solvers often lack free-surface capabilities, rendering them unsuitable for open-channel applications \parencite{chauchat_sedfoam-20_2017, studenik_openhfdib-dem_2024}.\par

This study's primary significance lies in bridging this critical gap by developing a numerical solver that integrates four key technologies into a single, cohesive framework within OpenFOAM: a Discrete Element Method (DEM) for arbitrary convex polyhedra, an Immersed Boundary Method (IBM) for fluid-solid coupling, Reynolds-Averaged Navier-Stokes (RANS) equations for turbulence closure, and the Volume of Fluid (VOF) method for free-surface tracking. This research seek to deliver a validated, open-source computational tool that enhances predictive capabilities for studying sediment dynamics. It aims to provide a more physically realistic framework for simulating the complex interplay between fluid turbulence, individual particle kinematics, and free-surface effects, thereby contributing to better-informed strategies in environmental and engineering applications.\par

\section{Scope and Limitations}

This study focuses on developing an OpenFOAM-based CFD-DEM solver for sediment transport modeling using RANS turbulence closure schemes. The research scope encompasses the study of existing solvers, development of a coupled CFD-DEM framework, and validation against fundamental test cases and data from literature.\par
